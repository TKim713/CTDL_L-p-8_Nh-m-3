...
struct ThreadedBinaryTree {
    ...
    void NodeInfo(NODE* root, int a, int b, int noderatio, int key, int level, int tmp) {
        ...
        if (root->LLINK != nullptr && root->LTAG == false) {
            if (root->level == 0)
                NodeInfo(root->LLINK, a - tmp * noderatio - 45, b + 90, noderatio - 1, key, level, tmp - 10);
            else NodeInfo(root->LLINK, a - tmp * noderatio + 5, b + 90, noderatio - 1, key, level, tmp - 5);
        }
        if (root->RLINK != nullptr && root->RTAG == false) {
            if (root->level == 0)
                NodeInfo(root->RLINK, a + tmp * noderatio + 45, b + 90, noderatio - 1, key, level, tmp - 10);
            else NodeInfo(root->RLINK, a + tmp * noderatio - 5, b + 90, noderatio - 1, key, level, tmp - 5);
        }
    }
    ...
    NODE* insert(NODE* root, int key) {
        ...
        while (ptr != nullptr) {
            ...
            if (key == (ptr->INFO)) {
                ...
                DrawNodeColor(ptr->x, ptr->y, key, 11);
                DrawNodeColor(x - 300, y - 150, ptr->INFO, 15);
                DrawNodeColor(x - 500, y - 150, key, 15);
    ...
    NODE* search(NODE* root, int key) {
        ...
        while (curr != nullptr) {
            ...
            if (curr->INFO == key) {
                outtextxy(x - 400, y - 160,(char*) "=");
                DrawNodeColor(curr->x, curr->y, key, 11);
                DrawNodeColor(x - 300, y - 150, curr->INFO, 15);
                DrawNodeColor(x - 500, y - 150, key, 15);
                DrawNodeColor(x - 500, y - 70, key, 15);
                setcolor(LIGHTGREEN);
