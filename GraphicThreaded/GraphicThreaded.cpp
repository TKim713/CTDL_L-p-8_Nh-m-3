...
struct ThreadedBinaryTree {
    ...
    void NodeInfo(NODE* root, int a, int b, int noderatio, int key, int level, int tmp) {
        root->x = a;
        root->y = b;
        root->level = level;
        level++;
        if (root->LLINK != nullptr && root->LTAG == false) {
            if (root->level == 0)
                NodeInfo(root->LLINK, a - tmp * noderatio - 45, b + tmpy + 10, noderatio - 1, key, level, tmp - 10);
            else NodeInfo(root->LLINK, a - tmp * noderatio + 5, b + tmpy + 10, noderatio - 1, key, level, tmp - 5);
        }
        if (root->RLINK != nullptr && root->RTAG == false) {
            if (root->level == 0)
                NodeInfo(root->RLINK, a + tmp * noderatio + 45, b + tmpy + 10, noderatio - 1, key, level, tmp - 10);
            else NodeInfo(root->RLINK, a + tmp * noderatio - 5, b + tmpy + 10, noderatio - 1, key, level, tmp - 5);
        }
    }

    // First Node appears to move
    void DrawStartNode(int a, int b, int key, int color) {
        setcolor(color);
    ...
    void CompareNode(NODE* root, int a, int b, int key) {
        ...
        if (key > root->INFO) {
            ...
            DrawStartNode(a + 200, b, root->INFO, 15);
            DrawStartNode(root->x, root->y, root->INFO, 11);
            delay(1000);
            if (root->RTAG == false) {
                setlinestyle(0, 0, 1);
                setcolor(LIGHTCYAN);
    ...
    void MoveNode(NODE* root, int a, int b, int key, int level) {
        setcolor(WHITE);
        DrawStartNode(a, b, key, 15);
        char* node;
        ...
        CompareNode(root, a, b, key);
        
        // If root->level > level: move down
        while (root->level >= level && root->INFO == key) {
            for (int j = 0; j <= (root->y - b); j += 2)
            {
        ...
        if (root->LLINK != nullptr && root->LTAG == false && key < root->INFO) {
            MoveNode(root->LLINK, x - 500, y - 150, key, root->level);
        }
        else if (root->RLINK != nullptr && root->RTAG == false && key > root->INFO) {
            MoveNode(root->RLINK, x - 500, y - 150, key, root->level);
        }
    }
    ...
    NODE* insert(NODE* root, int key) {
        ...
        while (ptr != nullptr) {
            ...
            if (key == (ptr->INFO)) {
                outtextxy(x - 400, y - 160,(char*) "=");
                DrawStartNode(ptr->x, ptr->y, key, 11);
                DrawStartNode(x - 300, y - 150, ptr->INFO, 15);
                DrawStartNode(x - 500, y - 150, key, 15);
                DrawStartNode(x - 500, y - 70, key, 15);
                delay(1000);
                setcolor(LIGHTRED);
    ...
    NODE* search(NODE* root, int key) {
        ...
        while (curr != nullptr) {
            ...
            if (curr->INFO == key) {
                outtextxy(x - 400, y - 160,(char*) "=");
                DrawStartNode(curr->x, curr->y, key, 11);
                DrawStartNode(x - 300, y - 150, curr->INFO, 15);
                DrawStartNode(x - 500, y - 150, key, 15);
                DrawStartNode(x - 500, y - 70, key, 15);
                setcolor(LIGHTGREEN);
    ...
    void Inorder(NODE* root) {
        ...
        while (ptr != nullptr) {
            ...
            bar(ptr->x - NODE_SIZE, ptr->y - NODE_SIZE, ptr->x + NODE_SIZE, ptr->y + NODE_SIZE);
            if (ptr->INFO >= 10)
                outtextxy(ptr->x - 10, ptr->y - 8, arr);
            else outtextxy(ptr->x - 5, ptr->y - 8, arr);
    ...
    NODE*preorderSuccessor(NODE *ptr) {
        if (ptr->LTAG == false) {
            ...
            setlinestyle(0, 0, 1);
            line(ptr->x + NODE_SIZE * 2, ptr->y, ptr->RLINK->x + 3, ptr->RLINK->y - 30 + NODE_SIZE);
            delay(1000);
            return ptr->RLINK;
        }
        else {
    ...
    NODE* PostorderNextNode(NODE* P)
    {
        ...
        if (P->LLINK == nullptr && P->LTAG == true) {
            ...
            while (P->LLINK != nullptr && P->LTAG == true) {
                setcolor(LIGHTCYAN);
                setlinestyle(3, 0, 1);
    ...
    void Postorder(NODE* root, int count) {
        ...
        while (P != nullptr) {
            ...
            setcolor(LIGHTCYAN);
            setfillstyle(SOLID_FILL, LIGHTCYAN);
            bar(P->x - NODE_SIZE, P->y - NODE_SIZE, P->x + NODE_SIZE, P->y + NODE_SIZE);
            if (P->INFO >= 10)
                outtextxy(P->x - 10, P->y - 8, key1);
            else outtextxy(P->x - 5, P->y - 8, key1);
            delay(1000);
            i -= 50;
            ...
            while (P->RTAG == false) {
                ...
                bar(P->x - NODE_SIZE, P->y - NODE_SIZE, P->x + NODE_SIZE, P->y + NODE_SIZE);
                if (P->INFO >= 10)
                    outtextxy(P->x - 10, P->y - 8, key2);
                else outtextxy(P->x - 5, P->y - 8, key2);
                delay(1000);
                i -= 50;
...
Menu:
    ...
    case 1: {
        ...
        if (c == true) {
            ...
            settextstyle(10, HORIZ_DIR, 3);
            tbt.NodeInfo(root, x, y, 8, item, 0, tmp);
            tbt.MoveNode(root, x - 500, y - 150, item, root->level);
